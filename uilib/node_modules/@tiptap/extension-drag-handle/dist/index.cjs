...
function getActiveDragRange(selection) {
  var _a;
  if (!(0, import_extension_node_range2.isNodeRangeSelection)(selection)) {
    return null;
  }
  return {
...
function createDroppedNodeRangeSelection(doc, anchorPos, nodeCount, depth) {
  try {
    ...
    const selection = import_extension_node_range2.NodeRangeSelection.create(doc, range.anchor, range.head, depth);
...
var getAbsolutePos = (state, relativePos) => {
  ...
  return (0, import_y_tiptap.relativePositionToAbsolutePosition)(
    ystate.doc,
    ystate.type,
    relativePos,
